...
final class GameStateManager: ObservableObject {
    ...
    init(hapticsManager: HapticsManager, detectHand: @escaping ([Card]) -> HandType? = PokerHandDetector.detectHand) {
    ...
    func dealInitialCards() {
        ...
        if deck.count >= GridConstants.initialCardCount {
             ...
             print("Deck has sufficient cards (\(deck.count)). Dealing \(GridConstants.initialCardCount).")
            ...
            print("⚠️ Warning: Deck only has \(deck.count) cards. Dealing available cards.")
    ...
    func removeCardsAndShiftAndDeal(playedCards: [Card]) async {
        ...
        print("🔍 Animating removal of \(playedCards.count) cards. Positions: \(emptyPositions)")
        ...
        // 2. REMOVED: Wait for the removal animation to complete
        //    We let the shift animation start immediately after the removal animation is triggered.
        /*
        do {
            ...
            print("⏱️ Removal Task.sleep interrupted: \(error)")
        }
        */

        // 3. Actually remove the cards from the data source
        cardPositions.removeAll { $0.isBeingRemoved }
        print("🔍 Remaining cards after removal, before shift: \(cardPositions.count)")
        ...
        do {
            ...
            print("⏱️ Shift Task.sleep interrupted: \(error)") 
        }
